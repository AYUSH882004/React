    ...
    <div className="bg-black h-screen flex items-start gap-10 text-white p-10">
      ...
      <div className="pl-6">
        ...
        <div className='flex flex-wrap gap-5 my-14 text-black'>
          {task.map((item, idx) => (
            <div key={idx} className='relative h-70 w-60 bg-white rounded-xl p-3 flex flex-col gap-4'>
              <h3 className="text-3xl underline font-bold">{item.heading}</h3>
              <p className="leading-tight text-xl text-gray-600 font-semibold">{item.title}</p>
              <button onClick={()=>DeleteTask(idx)} className='absolute active:scale-95 bottom-2 left-9 bg-red-400 py-1 px-9 text-xl font-semibold rounded-md'>Delete</button>
            </div>
          ))}
